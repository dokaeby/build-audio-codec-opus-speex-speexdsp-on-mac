...
SDKVERSION_MAC="12.1" 
ARCHS_DEVICES_MACX="x86_64 x86_64h arm64 arm64e"
...
if [ ! -e "${SRCDIR}/speexdsp-${VERSION}.tar.gz" ]; then
	echo "Downloading speexdsp-${VERSION}.tar.gz"
	curl -LO http://downloads.xiph.org/releases/speex/speexdsp-${VERSION}.tar.gz
fi
echo "Using speexdsp-${VERSION}.tar.gz"

tar zxf speexdsp-${VERSION}.tar.gz -C $SRCDIR
cd "${SRCDIR}/speexdsp-${VERSION}"
...
# These are the libs that comprise libspeexdsp.
OUTPUT_LIB="libspeexdsp.a"
INPUT_LIBS=""
